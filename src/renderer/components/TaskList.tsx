...
interface TaskProps {
  ...
  onSetAsPreview?: (imageUrl: string) => void;
}

const TaskItem = React.memo(({ task, onRefresh, onDelete, onPreview, onSetAsPreview }: TaskProps) => {
...
interface TaskListProps {
  ...
  onSetPreviewForPrompt?: (promptId: string, imageUrl: string) => void;
}

const TaskList: React.FC<TaskListProps> = ({ tasks, prompts, onRefresh, onDelete, onClearFinished, onSetPreviewForPrompt }) => {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {pickingPreviewFor && prompts && onSetPreviewForPrompt && (
        ...
        >
          <div
            className="bg-slate-800 border border-slate-600 rounded-xl shadow-2xl w-96 max-h-[70vh] flex flex-col"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="px-4 py-3 border-b border-slate-700 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-slate-200">选择提示词设置预览图</h3>
              <button
                onClick={() => { setPickingPreviewFor(null); setPromptSearch(''); }}
            ...
            <div className="px-4 py-2 border-b border-slate-700">
              <input
                ...
                className="w-full px-3 py-1.5 bg-slate-900 border border-slate-600 rounded text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:border-purple-500"
                autoFocus
              />
            </div>
              ...
              {prompts
                ...
                .map(p => (
                  <button
                    key={p.id}
                    onClick={() => {
                      onSetPreviewForPrompt(p.id, pickingPreviewFor);
                      setPickingPreviewFor(null);
                      setPromptSearch('');
                    }}
                    className="w-full text-left px-3 py-2 rounded-lg hover:bg-slate-700 transition-colors flex items-center gap-3"
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[10px] text-slate-500 truncate">{p.content}</p>
                    </div>
                  </button>
                ))}
