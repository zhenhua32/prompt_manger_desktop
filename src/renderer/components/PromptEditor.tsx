...
const PromptEditor: React.FC<PromptEditorProps> = ({
  ...
  const handleTranslate = async () => {
    ...
    if (!polishEnabled) {
      alert('请先在设置页面中启用并配置大模型 API');
      return;
    }
    setIsTranslating(true);
    try {
      const result = await onTranslate(content);
      setContentTranslation(result);
    } catch (error) {
      console.error('Translation failed:', error);
      alert('翻译失败: ' + (error as any).message);
    } finally {
  ...
  return (
    <div className="w-[500px] h-full bg-slate-800 border-l border-slate-700 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <label className="block text-sm font-medium text-slate-400">内容翻译</label>
            {onTranslate && (
              <button
                onClick={handleTranslate}
                disabled={isTranslating || !content.trim()}
                className="flex items-center gap-1 px-2 py-1 text-xs rounded bg-blue-600 hover:bg-blue-500 text-white disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
              >
                {isTranslating ? (
                  <>
